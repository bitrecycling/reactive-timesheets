...
function createActivityEntries1(){
...
function createActivityEntries2(){
...
function createActivityEntries3(){
...
echo "taskentries created for activity${TASK1_ID}"
curl -s "http://localhost:8080/taskentries?taskId=${TASK1_ID}" | jq length
echo "taskentries created for activity${TASK2_ID}"
curl -s "http://localhost:8080/taskentries?taskId=${TASK2_ID}" | jq length
echo "taskentries created for activity${TASK3_ID}"
...
function printActivityEntries(){
    IFS=' ' read -ra yo <<< "$1"
    echo "Activity'${yo[1]} ${yo[2]}'"
