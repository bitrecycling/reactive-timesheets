...
public class TimeshizzClientLibApplicationTests {
    ...
    public void testClient(){
        ...
        StepVerifier.create(webClient.post().uri("/clients").body(
                BodyInserters.fromFormData("name","clientLibClientName").with("address","clientLibClientAddress")
        ).exchange())
                .assertNext(
                        clientResponse -> {
                            Assert.assertTrue(clientResponse.statusCode().is2xxSuccessful());
                        }
                ).verifyComplete();
    }
}
